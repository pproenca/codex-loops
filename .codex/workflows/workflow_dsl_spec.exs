defmodule WorkflowDslSpecWorkflow do
  ...
  workflow "workflow-dsl-spec" do
    ...
    log(
      "SPEC.md already exists — mapping current structure, §10 dataflow, and §11 authoring guide for maintenance"
    )
    ...
    refine(:draft,
      reviewers: [
        ...
        reviewer(
          ...
          LENS: INVARIANTS. A defect is any place the spec states or implies something that violates: inert closure-free tree, determinism-by-absence, journal-as-sole-truth (no process state), exactly-once effects, bounded/terminating loops, validate-in-parse-at-compile-time. Also flag any execution algorithm whose determinism or replay-safety is not actually guaranteed by what the spec says. NOTE the §10 dataflow core adopts the journaled-values-only, deterministic-render-only rule as current normative behavior; do NOT flag value binding itself as an invariant violation, but DO verify the rule preserves the listed invariants — flag any dataflow construct whose closure-freedom, determinism-by-absence, journal-as-sole-truth, exactly-once, or bounded termination is not actually guaranteed by what the spec says (e.g. an unpinned inspect-map render order, a template hole that could admit non-journaled data, a bound value read before its producer commits, an unbounded map).
