defmodule Workflow.Status do
  ...
  defp apply_known_event(%Event{type: :refine_input_invalid, payload: p}, s) do
    tick(%{
      s
      | state: :failed,
        failure: %{address: p.address, iteration: 0, attempts: 0, reason: {:invalid_refine_input, p.address, p.reason}}
    })
  end
  ...
  defp apply_known_event(%Event{type: :refine_role_failed, payload: p}, s) do
    tick(%{s | usage: add_failed_usage(s.usage, Map.get(p, :usage))})
  end
  ...
  defp apply_known_event(%Event{type: type}, s) when type in [:iteration_started, :loop_decision, :loop_completed],
    do: tick(s)
  ...
  defp apply_known_event(%Event{type: type}, s)
       when type in [:verify_started, :judge_started, :fan_out_started, :fan_out_completed], do: tick(s)
  ...
  defp append_tool_activity(%__MODULE__{} = status, %Event{type: :agent_activity, payload: p} = event) do
    entry = maybe_put_activity_index(p.entry, Map.get(p, :activity_index))
  ...
  defp append_tool_activity(%__MODULE__{} = status, %Event{type: type, payload: p} = event)
       when type in [:agent_committed, :agent_attempt_rejected, :agent_failed, :refine_role_failed] do
  ...
  defp update_refines(%__MODULE__{} = status, %Event{type: :refine_round_started, payload: p} = event) do
    upsert_refine(status, p.address, fn refine ->
      refine
      |> Map.put(:state, :running)
  ...
  defp update_refines(%__MODULE__{} = status, %Event{type: :refine_gate_evaluated, payload: p} = event) do
    upsert_refine(status, p.address, fn refine ->
  ...
  defp update_refines(%__MODULE__{} = status, %Event{type: :refine_input_invalid, payload: p} = event) do
    upsert_refine(status, p.address, fn refine ->
      refine
  ...
  defp raw_ref(%__MODULE__{} = status, %Event{} = event) do
    put_present(
      %{run_id: event.run_id || status.run_id, seq: event.seq, type: Atom.to_string(event.type)},
      :address,
      Map.get(event.payload, :address)
    )
  end
