defmodule WorkflowDslSpecWorkflow do
  ...
  workflow "workflow-dsl-spec" do
    ...
    while_budget reserve: 0, until: count(:consensus) >= 1, max_iterations: 5 do
      log("Convergence round: adversarial panel reviewing SPEC.md")
      ...
      agent(
        ...
        Adversarially review the CURRENT SPEC.md at the repository root (read it fresh; read .spec-workshop/ and the committed HEAD version as your lens requires). Assume the delta is defective until proven otherwise. Return pass=false with concrete, located defects if you find ANY in scope; pass=true only if this lens is fully satisfied.
        """,
        label: "review:spec-completeness"
      )
      ...
      agent(
        ...
        Adversarially review the CURRENT SPEC.md at the repository root (read it fresh; read .spec-workshop/ and the committed HEAD version as your lens requires). Assume the delta is defective until proven otherwise. Return pass=false with concrete, located defects if you find ANY in scope; pass=true only if this lens is fully satisfied.
        """,
        label: "review:non-destructiveness"
      )

      agent(
        """
        CONSENSUS GATE. Read the six immediately preceding lens review outputs in this run.

        Return JSON only:
        - [] if any lens returned pass=false, if any lens output is missing, or if you cannot prove unanimity.
        - [{"id":"unanimous","verdict":"pass"}] if and only if all six lenses returned pass=true.

        Do not edit files. This gate exists only to turn unanimous agreement into a journaled accumulator marker so the workflow loop can stop before the five-round cap.
        """,
      ...
      collect(into: :consensus)

      log(
        "Convergence round: panel review outputs are journaled; reviser should resolve any open defects"
      )
      ...
      agent(
        ...
        Read the immediately preceding CONSENSUS GATE output first. If it returned [{"id":"unanimous","verdict":"pass"}], make no edits and report convergence.

        Otherwise, the adversarial panel found defects in the §10 dataflow insertion. Resolve EVERY defect below with TARGETED edits. This remains a surgical
