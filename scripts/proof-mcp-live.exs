defmodule ProofMCPLive do
  ...
  def run do
    ...
    temp_root =
      Path.join(
        System.tmp_dir!(),
        "codex-loops-mcp-live-proof-#{System.unique_integer([:positive])}"
      )
  ...
  defp do_poll_completed_status!(client, run_id, _next_id, 0, last_payload) do
    inspected =
      try do
        case call_tool!(client, 900, "workflow_inspect", %{"run_id" => run_id}) do
          {response, _client} -> response["result"] && response["result"]["structuredContent"]
        end
      rescue
